...
function localSiblingAliases(): Record<string, string> {
  const candidates: Record<string, string> = {
    // @mongez/dotenv has no `@mongez/*` runtime deps today. The list is
    // kept here so future siblings (e.g. a typed-env helper) get picked up
    // automatically when they appear next to this package.
  };
